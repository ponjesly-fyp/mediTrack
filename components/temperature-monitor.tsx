...
export default function TemperatureMonitor({ temperature }: { temperature: number }) {
    ...
    const colorClasses = getColorClasses(temperature)
    const minTemp = 95
    const maxTemp = 106
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <div className="text-right flex flex-col justify-end items-end">
                            <div className="flex items-baseline">
                                {temperature >= 90 && temperature <= 120 ? <><span className="text-3xl font-semibold text-gray-900">{temperature.toFixed(1)}</span>
                                <span className="text-sm ml-1 text-gray-500">°F</span></>:<h1 className="font-semibold">
                                    Disconnected</h1>}
                            </div>
                            {temperature >= 90 && temperature <= 120 ? <><span className="text-xs text-gray-500 hidden md:flex">{toCelsius(temperature).toFixed(1)}°C</span></>:
